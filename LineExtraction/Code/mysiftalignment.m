...
matches = match(I1,I2);
loc1match = loc1(matches(1,:),:);
des1match = des1(matches(1,:),:);
loc2match = loc2(matches(2,:),:);
des2match = des2(matches(2,:),:);

% Screen and obtain correct matches
indices = screenmatches(I1,I2,matches,loc1match,des1match,loc2match,des2match);
n = size(indices,2); % number of correct matches